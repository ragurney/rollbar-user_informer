...
describe Rollbar::UserInformer do
  ...
  let(:app) { Rollbar::UserInformer::Middleware.new(->(_env) { response }) }
  let(:call) { app.call(env) }
  let(:expected_body_with_replacement) { 'Original before 1234 after Body' }

  before { Rollbar::UserInformer.user_information = 'before {{error_uuid}} after' }
  ...
  describe '#call' do
    ...
    describe 'using custom placeholder' do
      ...
      it 'can use custom placeholder' do
        original_placeholder = Rollbar::UserInformer.user_information_placeholder
        begin
          Rollbar::UserInformer.user_information_placeholder = "<!-- CUSTOM PLACEHOLDER -->"
          call.must_equal(
            [
              200,
              { "Content-Length" => "40", "Error-Id" => "1234" },
          ...
          Rollbar::UserInformer.user_information_placeholder = original_placeholder
        end
      end
    end
